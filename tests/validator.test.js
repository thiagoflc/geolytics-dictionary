...
describe('SPE_PRMS_INVALID_CATEGORY', () => {
  test('flags 4P as invalid SPE-PRMS category', () => {
    ...
    assert.ok(hasViolation(result, 'SPE_PRMS_INVALID_CATEGORY'), 'expected SPE_PRMS_INVALID_CATEGORY violation');
  });
...
describe('RESERVA_AMBIGUITY', () => {
  test('flags reserva O&G text with environmental signal REBIO', () => {
    const result = validate('A reserva petrolífera 2P fica próxima à REBIO Comboios no campo de Espírito Santo');
...
describe('OSDU_KIND_FORMAT', () => {
  ...
  test('accepts canonical OSDU Field kind', () => {
    const result = validate('OSDU kind opendes:osdu:master-data--Field:1.0.0 para campos de petróleo');
    assert.ok(!hasViolation(result, 'OSDU_KIND_FORMAT'));
  });
});
...
describe('LAYER_COVERAGE_MISMATCH', () => {
  test('flags entity in layer not in its geocoverage', () => {
    const claim = { type: 'entity', value: 'poco em layer9', context: { entity_id: 'poco', layer: 'layer9' } };
    const result = validate(claim);
    assert.ok(hasWarning(result, 'LAYER_COVERAGE_MISMATCH'), 'should warn for layer not in geocoverage');
  });

  test('does not flag entity in valid layer', () => {
    const claim = { type: 'entity', value: 'poco em layer1', context: { entity_id: 'poco', layer: 'layer1' } };
    const result = validate(claim);
