...
const entityGraph = JSON.parse(
  fs.readFileSync(path.join(ROOT, "data", "entity-graph.json"), "utf8")
);
const ontopetro = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "ontopetro.json"), "utf8"));
const crosswalk = JSON.parse(
  fs.readFileSync(path.join(ROOT, "data", "osdu-gso-crosswalk.json"), "utf8")
);
...
ontopetro.relations.forEach((rel) => {
  ...
  if (domainClass && rangeClass) {
    emitRel(`ontopetro_class_${domainClass.id}`, `ontopetro_class_${rangeClass.id}`, rel.name, {
...
crosswalk.mappings.forEach((m) => {
  ...
  if (!osduClass) {
    lines.rels.push(`// GSO-only: ${gsoClass} (${m.match}) — no OSDU equivalent`, "");
    return;
  }
...
fs.writeFileSync(
  path.join(OUT, "relationships.cypher"),
  relHeader.join("\n") + lines.rels.join("\n")
);
