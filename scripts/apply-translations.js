...
function validateRows(rows, filename) {
  ...
  for (const r of rows) {
    ...
    if (!APPROVED_SOURCES.has(r.source)) {
      errs.push(
        `${filename}: ${r.id} — source='${r.source}' not in approved set (run promote step)`
      );
    }
  }
  return errs;
...
console.log(
  `Loaded ${lithoPt.length + lithoEs.length + petroPt.length + petroEs.length} translation rows — all validated.`
);
...
for (const [scheme, info] of Object.entries(petroDoc.data)) {
  for (const c of info.concepts) {
    ...
    petroTtlLines.push(
      `    skos:prefLabel "${escTtl(tr[c.id].pt)}"@pt , "${escTtl(tr[c.id].es)}"@es .`
    );
    petroTtlLines.push("");
    petroTtlCount++;
  }
...
console.log(
  `geolytics-petrography-i18n.ttl: ${petroTtlCount} concepts × 2 lang = ${petroTtlCount * 2} prefLabel triples`
);
...
console.log(
  `geolytics-glosis-ext.ttl: ${i18nIdx === -1 ? "appended" : "replaced"} ${lithoTtlCount} concepts × 2 = ${lithoTtlCount * 2} prefLabel triples`
);
