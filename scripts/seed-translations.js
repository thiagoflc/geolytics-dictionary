...
const CGNT = new Map([
  ...
  [
    ...
    ["andesito, traquito, fonolito", "andesita, traquita, fonolita"],
  ],
  ...
  [
    ...
    ["ilmenita, magnetita, ironstone, serpentina", "ilmenita, magnetita, ironstone, serpentina"],
  ],
  // Metamorphic
  ...
  [
    ...
    ["ardósia, filito (rochas pelíticas)", "pizarra, filita (rocas pelíticas)"],
  ],
  ["schist", ["xisto", "esquisto"]],
  ["(green)schist", ["xisto verde", "esquisto verde"]],
  [
    "gneiss rich in fe–mg minerals",
    ["gnaisse rico em minerais Fe–Mg", "gneis rico en minerales Fe–Mg"],
  ],
  [
    "metamorphic limestone (marble)",
    ["mármore (calcário metamórfico)", "mármol (caliza metamórfica)"],
  ],
  ...
  [
    "sedimentary rock (consolidated)",
    ["rocha sedimentar (consolidada)", "roca sedimentaria (consolidada)"],
  ],
  ...
  [
    ...
    ["calcário, outras carbonáticas", "caliza, otras carbonatadas"],
  ],
  ["marl and other mixtures", ["marga e outras misturas", "marga y otras mezclas"]],
  [
    "coals, bitumen and related rocks",
    ["carvões, betume e correlatas", "carbones, betún y rocas afines"],
  ],
  // Unconsolidated
  [
    "sedimentary rock (unconsolidated)",
    ["sedimento (não consolidado)", "sedimento (no consolidado)"],
  ],
  ["anthropogenic/technogenic", ["antropogênico/tecnogênico", "antropogénico/tecnogénico"]],
  [
    "redeposited natural material",
    ["material natural redepositado", "material natural redepositado"],
  ],
  [
    "industrial/artisanal deposits",
    ["depósitos industriais/artesanais", "depósitos industriales/artesanales"],
  ],
  ...
  [
    ...
    ["camada de solifluxão periglacial", "capa de solifluxión periglaciar"],
  ],
  ...
  [
    ...
    ["turfa de pântano alimentada por chuva", "turba de páramo alimentada por lluvia"],
  ],
  [
    "groundwater-fed bog peat",
    ["turfa de brejo alimentada por aquífero", "turba de ciénaga alimentada por acuífero"],
  ],
  ...
  [
    "non-cemented and non-compacted",
    ["não cimentado e não compactado", "no cementado y no compactado"],
  ],
  ...
  [
    ...
    ["ferro–manganês (sesquióxidos)", "hierro–manganeso (sesquióxidos)"],
  ],
  ...
  [
    ...
    ["fresco ou levemente intemperizado", "fresco o ligeramente meteorizado"],
  ],
  ...
  [
    "fine and medium gravel/artefacts",
    ["cascalho/artefatos finos e médios", "grava/artefactos finos y medios"],
  ],
  [
    "medium and coarse gravel/artefacts",
    ["cascalho/artefatos médios e grossos", "grava/artefactos medios y gruesos"],
  ],
  ...
  [
    ...
    ["zona de escarpa de gradiente médio", "zona de escarpe de gradiente medio"],
  ],
  ...
  [
    "fragmentsvolumetricestimate",
    ["estimativa volumétrica de fragmentos", "estimación volumétrica de fragmentos"],
  ],
  ...
  [
    ...
    ["zona de escarpa de gradiente alto", "zona de escarpe de gradiente alto"],
  ],
...
function loadLithology() {
  const data = JSON.parse(
    fs.readFileSync(path.join(ROOT, "data/glosis/lithology.json"), "utf8")
  ).data;
...
function loadPetrography() {
  const data = JSON.parse(
    fs.readFileSync(path.join(ROOT, "data/glosis/petrography-codelists.json"), "utf8")
  ).data;
  const out = [];
...
for (const [name, concepts] of [
  ...
  for (const lang of ["pt", "es"]) {
    ...
    console.log(
      `  ${name}.${lang}.csv: ${stats.total} rows  curated=${stats.curated}  heuristic=${stats.heuristics}  blank=${stats.blank}`
    );
  }
}
