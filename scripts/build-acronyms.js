...
function stripTrailingMeta(s) {
  ...
  return s
    .replace(/^(Port\.|Ang\.|Portugal|Brasil|Brazil|Angola)(?=[,;\s.\-)]|$)[,;\s.\-)]*/i, "")
    .trim();
}
...
function parseLine(rawLine) {
  ...
  if (altRaw) {
    ...
    if (mainIsPT && !altIsPT) {
      pt = main;
      en = altRaw;
    } else if (!mainIsPT && altIsPT) {
      en = main;
      pt = altRaw;
    } else if (mainIsPT && altIsPT) {
      pt = main;
      en = altRaw; /* ambíguo, assume primário=PT */
    } else {
      en = main;
      pt = altRaw;
    }
